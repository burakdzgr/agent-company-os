...
> | **B1** router pricing | ✅ uygulandı (Option B) | Rapordaki premis yanlıştı: `model_providers.pricing` sütunu **yok**; ayrıca doc 26 §3.1 **model-bazlı** fiyat istiyor, router sağlayıcı-bazlı anahtarlıyordu (opus/haiku 5x sapma). Şema değişikliği olmadan çözüldü: `pricing-defaults.ts` (model-keyed, doc 26 şekli) + `resolveProviderPricing(entry, model)` + router lookup'ı model-farkındalı (eski şekil geriye uyumlu). `logCall` no-op'ları gerçek loglamaya bağlandı — **başarısız denemeler de** kayda geçiyor (ADR-015) + fiyatsız model uyarısı. 10 regresyon testi: aynı sağlayıcı satırında sonnet 1800¢ / haiku 600¢ / opus 3000¢ farklı çıkmalı. Scripted hat da fiyatlı ⇒ e2e artık `recordCost` → `cost_entries` → bütçe guard → devre kesici zincirini gerçekten yürütüyor. **Açık kalan:** runtime düzenlenebilirlik (Option A: migration + Settings → Providers) |
> | **Y1** request_help/record_decision | ✅ uygulandı | `request_help` → görev kanalına `help_request` mesajı + SignalPort teslimi (INV-16 yardım yolu açıldı); `record_decision` → `artifacts` tablosuna ADR benzeri karar kaydı. `default` dalı artık **exhaustiveness guard** (13 aksiyonun tamamı bağlı; şemaya yeni tip eklenirse derleme kırılır) |
