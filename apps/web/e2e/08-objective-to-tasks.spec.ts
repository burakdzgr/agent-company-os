...
test("Founder objective decomposes through the hierarchy; inbox stays empty", async ({ page }) => {
  ...
  // steps 9–11: the cascade materializes on the board via live WS invalidation
  await expect(
    page.getByText("Initiative: analyze the project and deliver feature X"),
  ).toBeVisible({ timeout: 90_000 });
  await expect(page.getByText("Epic: implement feature X behind the existing API")).toBeVisible({
    timeout: 90_000,
  });
  ...
  // owners work)
  await expect(
    page.getByTestId("column-Review").getByText("Implement the feature X service endpoint"),
  ).toBeVisible({ timeout: 90_000 });
  await expect(
    page.getByTestId("column-Review").getByText("Write integration coverage for feature X"),
  ).toBeVisible({ timeout: 90_000 });
