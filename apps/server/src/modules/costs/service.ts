// Cost management (26 §1,4,5; T28). cost_entries is an append-only ledger
// (corrections = compensating negative entries, never updates). Budgets are
// windowed queries — no row mutation on period reset. Breaches are detected
// on the crossing write and emit budget.warning (soft) / budget.exceeded
// (hard); a company-scope hard breach trips the circuit breaker, pausing all
// non-critical agents (critical = lead/executive track, or owns a task in
// REVIEW/QA/APPROVAL, or employment.critical=true — 26 §5 [WRITER-DECISION]).
import { and, eq, sql } from "drizzle-orm";
import type { CompanyContext, GuardedDb, Tx } from "@acos/db";
import { agents, budgets, costEntries, positions, tasks } from "@acos/db/schema";
