// Delegation engine (07 §6–9; T28). No separate delegationWorkflow (09 §4):
// these are the create_task / delegate_task activity semantics the manager
// loop (T32) calls. Reporting-line rule, WIP capacity (no TOCTOU — checked in
// the assignment transaction), pro-rata budget inheritance (80/20), and the
// forced-manager-intervention path when the reassignment limit trips.
import { and, eq, isNull, sql } from "drizzle-orm";
import type { CompanyContext, GuardedDb } from "@acos/db";
import { agents, budgets, orgEdges, positions, tasks } from "@acos/db/schema";
