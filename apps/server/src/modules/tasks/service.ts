// Task engine (07 §1–5; _DECISIONS §7; T27). TasksService owns CRUD, the
// kind hierarchy, per-company numbers and the dependency DAG (cycle-checked
// on write). TaskStateService is the ONLY status writer: every transition
// runs in one transaction under a row lock, resolves the acting agent's
// actor classes server-side (never client-declared) and authorizes via the
// pure domain matrix before updating + emitting task.status.changed.
...
import { nextSequenceValue, type CompanyContext, type GuardedDb, type Tx } from "@acos/db";
import { agents, positions, taskAssignments, taskDependencies, tasks } from "@acos/db/schema";
